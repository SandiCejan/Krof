...
namespace Krof
{
    ...
    internal class MonsterBoss : AIActor
    {
        ...
        private bool ExplosiveDropped = false;
        bool lockMovement = false;
        bool caughtInWall = false;


        public MonsterBoss(Game game, Vector2 position) : base(position, new Vector2(.5f, .5f))
        {
            WalkSpeed = 3;
            RunSpeed = 5;
            collidersState = new int[] { 0, 0, 0, 0 };
            drawableItem = (DrawableItem)AddComponent(new DrawableItem(Renderer.Sprites[9]));
            ...
            //Renderer.drawableRectangleColliders.Add(collider);
            soundEffectWalk = SoundEngine.CreateAudioEmitter(5, Transform);
            soundEffectWalk.IsLooped = true;
            soundEffectWalk.Play();
            soundEffectRun = SoundEngine.CreateAudioEmitter(6, Transform);
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (!ExplosiveDropped && AnimFrame % 100 < 2)
            {
                new Explosive(Transform.Position + new Vector2((float)Math.Cos(Transform.Angle + Math.PI/2)*40, (float)Math.Sin(Transform.Angle + Math.PI / 2) * 40));
                ExplosiveDropped = true;
            }
            else
            {
                ExplosiveDropped = false;
            }
            //(Transform.Position - Player.player.Transform.Position).Length() < 2000 && 
            if (Physics.Raycast(Player.player.Transform.Position, Transform.Position, 1, false) == collider)
            {
